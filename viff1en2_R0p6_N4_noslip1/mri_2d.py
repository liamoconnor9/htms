...
grad_u = ex*(ex@d3.grad(u)) + ez*(ez@d3.grad(u)) + ex*lift(tau1u) # First-order reduction
grad_A = ex*(ex@d3.grad(A)) + ez*(ez@d3.grad(A)) + ex*lift(tau1A) # First-order reduction
grad_b = ex*(ex@d3.grad(b)) + ez*(ez@d3.grad(b))
