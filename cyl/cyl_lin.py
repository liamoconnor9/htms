...
# momentum-r
problem.add_equation("r**2 * dt(ur) + r**2 * dr(p) - r**2 * f * uth + nu * (ur  - r*(ur_r  + r*(dr(ur_r)  + dz(dz(ur)))))  + lift(tau_ur2)  - r**2 * (b_dg_b_r - u_dg_u_r) = r**2 * (C_dg_D_r(br, bth, bz, br, bth, bz) - C_dg_D_r(ur, uth, uz, ur, uth, uz))")

# momentum-theta
problem.add_equation("r**2 * dt(uth)   + r**2 * f * ur + nu * (uth - r*(uth_r + r*(dr(uth_r) + dz(dz(uth))))) + lift(tau_uth2) - r**2 * (b_dg_b_th - u_dg_u_th) = r**2 * (C_dg_D_th(br, bth, bz, br, bth, bz) - C_dg_D_th(ur, uth, uz, ur, uth, uz))")

# momentum-z
problem.add_equation("r    * dt(uz) + r    * dz(p) - nu * (r*dz(dz(uz)) + uz_r + r*dr(uz_r)) + lift(tau_uz2)               - r    * (b_dg_b_z - u_dg_u_z) = r * (C_dg_D_z(br, bth, bz, br, bth, bz) - C_dg_D_z(ur, uth, uz, ur, uth, uz))")
...
# induction-r
problem.add_equation("r**2 * dt(Ar) + r**2 * dr(phi) + eta * (Ar  - r*(Ar_r  + r*(dr(Ar_r)  + dz(dz(Ar)))))  + lift(tau_Ar2) - r**2 * (u_cross_b_r(U0r, U0th, U0z, br, bth, bz) + u_cross_b_r(ur, uth, uz, B0r, B0th, B0z) ) = r**2 * u_cross_b_r(ur, uth, uz, br, bth, bz)")

# induction-theta
problem.add_equation("r**2 * dt(Ath)                 + eta * (Ath - r*(Ath_r + r*(dr(Ath_r) + dz(dz(Ath))))) + lift(tau_Ath2) - r**2 * (u_cross_b_th(U0r, U0th, U0z, br, bth, bz) + u_cross_b_th(ur, uth, uz, B0r, B0th, B0z) ) = r**2 * u_cross_b_th(ur, uth, uz, br, bth, bz)")

# induction-z
problem.add_equation("r    * dt(Az) + r    * dz(phi) - eta * (r*dz(dz(Az)) + Az_r + r*dr(Az_r)) + lift(tau_Az2) - r * (u_cross_b_z(U0r, U0th, U0z, br, bth, bz) + u_cross_b_z(ur, uth, uz, B0r, B0th, B0z) ) = r * u_cross_b_z(ur, uth, uz, br, bth, bz)")
